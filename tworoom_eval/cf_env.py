...
def _variation_value_copy(space, key: str) -> np.ndarray:
    return np.asarray(space[key]["position"].value, dtype=int).copy()
...
def register_cf_env():
    ...
    if env_id not in gymnasium.envs.registry:
        gymnasium.register(
            id=env_id,
            entry_point="tworoom_eval.cf_env:TwoRoomCFEnv",
        )
